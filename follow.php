...
* Description: Adds a floating Follow Button at the bottom right corner of WordPress.ORG sites
* Version: 10.0.5
...
/*
Copyright (C) 2011-12 Aditya Jain

This file is part of the Follow Plugin

Follow is free plugin  : you can redistribute it and/or modify
...
along with followsubs. If not, see <http://www.gnu.org/licenses/>.
*/

if ( version_compare($GLOBALS['wp_version'], '3.1', '<') ) {
	// Follow Plugin needs WordPress 3.1 or above, exit if not on a compatible version
	$exit_msg = sprintf(__('This version of Follow Plugin requires WordPress 3.1 or greater. Please update %1$s.', 'follow'), '<a href="http://codex.wordpress.org/Updating_WordPress">Wordpress</a>', '<a href="http://wordpress.org/extend/plugins/follow/download/">Follow Plugin</a>');
	exit($exit_msg);
}

// our version number. Don't touch this or any line below
// unless you know exactly what you are doing
define( 'followpluginversion', '10.0.2' );
...
// variables and constructor are declared at the end
	...
	*/
	function load_strings() {
		...
		$this->not_an_email = "<p class=\"s2_error\">" . __('Sorry, but that does not look like an email address.', 'subscribe2') . "</p>";

		$this->barred_domain = "<p class=\"s2_error\">" . __('Sorry, email addresses at that domain are currently barred due to spam.', 'subscribe2') . "</p>";
...
/* ===== WordPress menu registration and scripts ===== */
	...
	*/
	function admin_menu() {
		add_menu_page (__('Follow Plugin', 'subscribe2'), __('Follow Plugin', 'subscribe2'), apply_filters('s2_capability', "read", 'user'), 's2', NULL, S2URL . 'include/star.png');
...
/* ===== Install, upgrade, reset ===== */
	...
	*/
	function upgrade() {
		...
		$this->subscribe2_options['version'] = followpluginversion;
...
/* ===== Menus ===== */
	...
	*/
	function options_menu() {
		...
		echo "<h2>" . __('Follow Plugin Settings', 'subscribe2') . "</h2>\r\n";
		echo "<a href=\"http://adityajain.name/\">" . __('Authors Blog', 'subscribe2') . "</a> | ";
		echo "<a href=\"http://forum.adityajain.name\">" . __('Authors Community Discussion Forum', 'subscribe2') . "</a> | ";
                echo "<a href=\"http://mywp.in/\">" . __('Plugin Website', 'subscribe2') . "</a>";
		echo "<form method=\"post\" action=\"\">\r\n";
		if ( function_exists('wp_nonce_field') ) {
		...
		echo __('Set default Subscribe page as (Page where you have inserted the S2 button)', 'subscribe2') . ': ';
		echo "<select name=\"page\">\r\n";
		$this->pages_dropdown($this->subscribe2_options['s2page']);
		echo "</select>\r\n";    echo "</select>\r\n";echo "<strong><em style=\"color: red\">" . __('In order to get a functional Follow floating button you must set this to "Subscribe" For installing instructions visit http://www.mywp.in and go to ReadMe page. The Permalink to the Subscribe page must be "your blog url address/subscribe"', 'subscribe2') . "</em></strong><br />\r\n";
...
echo __('Show the S2 button on the Write toolbar?', 'subscribe2') . "</label><br /><br />\r\n";                        echo "</select>\r\n";echo "<strong><em style=\"color: red\">" . __('This must be enabled (checked)', 'subscribe2') . "</em></strong><br />\r\n";
...
echo "<strong><em style=\"color: red\">" . __('This must be enabled (checked)', 'subscribe2') . "</em></strong><br />\r\n";
		...
		echo __('Enable Subscriber Counter Widget?', 'subscribe2') . "</label><br /><br />\r\n";
...
/* ===== Our constructor ===== */
	...
	*/
	...
	function subscribe2() {
		...
		load_plugin_textdomain('follow', false, S2DIR);
		...
		if ( is_array($this->subscribe2_options) && $this->subscribe2_options['version'] !== followpluginversion ) {
...
<style type="text/css" media="screen">
  ...
  #bit a.bsub span {
      ...
      background-image: url(<?php bloginfo('url' ); ?>/wp-content/plugins/follow/include/star.png);
...
<div id="bit" class="">
  ...
  <div id="bitsubscribe">
    ...
    <form action=<?php bloginfo('url' ); ?>/subscribe/ method="post" accept-charset="utf-8" id="loggedout-follow">
...
function ep_admin_init() {
	...
	global $wp_version;
	add_meta_box('ep_admin_meta_box', __( 'Follow Plugin : Exclude Pages', EP_TD ), 'ep_admin_sidebar_wp25', 'page', 'side', 'low');
